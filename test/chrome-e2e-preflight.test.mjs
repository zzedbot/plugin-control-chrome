...
const compatibility = { foreignFrameMonitor: "remove-after-blank-v11", debuggerState: "generation-v4" };
...
test("E2E preflight rejects old code before opening a test tab", async () => {
  for (const browser of [
    ...
    { version: VERSION },
    { version: VERSION, compatibility: { ...compatibility, debuggerState: "old" } }
  ]) {
...
test("E2E preflight returns running version evidence", async () => {
  const bridge = fixture({ version: VERSION, compatibility });
...
test("E2E preflight rejects a stale installed native host", async () => {
  const bridge = fixture({ version: VERSION, compatibility }, true, "0.1.0");
